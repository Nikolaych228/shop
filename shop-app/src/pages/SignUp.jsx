        ...
        try {
            ...
            if (password && email && firstName && lastName) {
                if (password.length >= 6) {
                    ...
                    setHelperText('пароль должен быть не короче 6 символов')
                    dispatch(removeRole())
                    return
                }
            }
            else{
                setHelperText('Вы ввели не все данные')
                dispatch(removeRole())
                return
            }
        } catch (error) {
            setLoading(false)
            setHelperText('Ошибка входа')
            dispatch(removeRole())
        } finally {
            setLoading(false)
        }
        ...
        <div class="md:flex w-full">
            ...
            <div class="w-full md:w-1/2 py-10 px-5 md:px-10">
                ...
                <div>
                    <div class="flex -mx-3">
                        <div class="w-1/2 px-3 mb-5">
                            ...
                            <div class="flex">
                                ...
                                <input onChange={(e) => {setFirstName(e.target.value)}} type="text" class="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="Иван"/>
                            </div>
                        </div>
                        <div class="w-1/2 px-3 mb-5">
                            ...
                            <div class="flex">
                                <div class="w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center"><i class="mdi mdi-account-outline text-gray-400 text-lg"></i></div>
                                <input onChange={(e) => {setLastName(e.target.value)}} type="text" class="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="Иванов"/>
                            </div>
                        </div>
                    </div>
                    <div class="flex -mx-3">
                    ...
                    <div class="flex -mx-3">
                        <div class="w-full px-3 mb-5">
                            <button onClick={registerHandler} disabled={loading} class="block w-full max-w-xs mx-auto bg-orange-500 hover:bg-orange-700 focus:bg-orange-700 text-white rounded-lg px-3 py-3 font-semibold">{loading ? 
                            <div className="w-full flex justify-center">
                                <div className="animate-spin inline-block w-6 h-6 border-[3px] border-current border-t-transparent text-white-600 rounded-full" role="status" aria-label="loading">
                                    <span className="sr-only">Loading...</span>
                                </div>
                            </div> : "Зарегистрироваться"}</button>
                            <div className="w-full flex justify-center mt-3">
                                <p>Есть аккаунт? <span onClick={() => {navigate('/signin')}} className='text-indigo-600 cursor-pointer'>Войти</span> </p>
                            </div>
                        </div>
                    </div>
                </div>
